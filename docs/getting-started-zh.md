...
- **第二行** — 一行标签条：项目 • **worktree pill**（只有图标，hover 或长按弹出 `worktree: <name>` 浮层，双击复制）• **Auto** 标签（开了 Auto 时才显示）• branch（紫色细字）• **Task** 标签（橙色，点开任务卡片）• **ID pill**（4 字符 monospace，hover/长按看浮层，双击复制，跟 worktree pill 一致）。
