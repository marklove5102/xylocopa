...
def _setup_syncing_agent(db, *, agent_id="synctest1111", project_name="disp-proj"):
    """Insert a SYNCING tmux agent with a PENDING message and return (agent, message)."""
...
async def test_liveness_check_skips_cli_sync_agents(db_engine, monkeypatch):
    """SYNCING tmux agents should not be stopped by _reap_dead_agents
...
@pytest.mark.anyio
async def test_liveness_check_skips_idle_agents_with_sync_task(db_engine, monkeypatch):
    """IDLE tmux agents with an active sync task should not be stopped by _reap_dead_agents."""
    from sqlalchemy.orm import sessionmaker
    from agent_dispatcher import AgentDispatcher
    import asyncio
    ...
    # Simulate an active sync task for this agent
    dispatcher._sync_tasks[agent.id] = asyncio.Future()
    ...
    # IDLE agent with sync task should remain IDLE
    assert agent.status == AgentStatus.IDLE
    db.close()
...
def test_agent_cli_sync_default_false(db_engine):
    """Model-level default cli_sync is False (all new agents set it True explicitly)."""
    from sqlalchemy.orm import sessionmaker
